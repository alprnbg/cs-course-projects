...
void print_list(LinkedList& list){
    Node* curr = list.head;
    cout<<"listenin adresi : "<<&list<<endl;
    while(curr){
        cout<<curr->name<<" "<<curr->amount<<"   "<<curr<<"   next : "<<curr->next<<endl;
        curr = curr->next;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
        ...
        LinkedList list1;
        list1.pushTail("alperen",10);
        list1.pushTail("emre",20);
        cout<<"list1"<<endl;
        print_list(list1);
        LinkedList list2(move(list1));
        cout<<"after move cons. list1"<<endl;
        print_list(list1);
        cout<<"list2"<<endl;
        print_list(list2);

        LinkedList list3;
        list3.pushTail("asd",133);
        list3.pushTail("asdasd",133);
        list1 = move(list3);
        cout<<"after move assign list1"<<endl;
        print_list(list1);
        cout<<"list 3"<<endl;
        print_list(list3);
